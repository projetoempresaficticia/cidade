...
2. Escreve `versao.json` e o `<meta name="cd-versao">` de cada
   página, com um resumo de TODOS os ficheiros versionados. Isto existe
   porque o ponto 1 sozinho não chega: o GitHub Pages manda
...
META = re.compile(r'<meta name="cd-versao" content="(?P<versao>[^"]*)"\s*/?>')
...
META_RESUMO = re.compile(r'\s*<meta name="cd-versao"[^>]*>')
    ...
    for html in paginas:
        ...
        if atual:
            if atual.group('versao') != versao_site:
                problemas += 1
                texto = META.sub(
                    f'<meta name="cd-versao" content="{versao_site}" />', texto, count=1)
        else:
            problemas += 1
            print(f'  sem meta de versão: {html.name}')
            texto = texto.replace(
                '<meta name="viewport"',
                f'<meta name="cd-versao" content="{versao_site}" />\n  <meta name="viewport"',
                1)
        textos[html] = texto
