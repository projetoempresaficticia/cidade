// Cidade — garante que ninguém fica preso numa versão velha. Mesmo
// mecanismo do resto do ecossistema: o GitHub Pages não deixa mudar o
...
(function () {
  const meta = document.querySelector('meta[name="cd-versao"]');
  if (!meta || !meta.content) return;
  const minha = meta.content;

  function jaTentei(versao) {
    const chave = 'cd-recarga:' + window.location.pathname + ':' + versao;
    try {
